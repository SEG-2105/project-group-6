...
public class PendingRequestsAdapterClient extends ArrayAdapter<Request> {
     ...
     */
    public PendingRequestsAdapterClient(@NonNull Context context, int resource, @NonNull List<Request> objects) {
    ...
    public View getView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
        ...
        if (convertView == null) { //must inflate view
            convertView = LayoutInflater.from(getContext()).inflate(R.layout.activity_completed_requests_list_item, parent, false);
        ...
        if (App.getClient() != null) {
            TextView requestStatus = convertView.findViewById(R.id.requestStatusField);
            // display request status for pending requests
            requestStatus.setVisibility(View.VISIBLE);
            if (!request.getIsPending() && !request.getIsCompleted() && !request.getIsRejected()) {
                requestStatus.setText(R.string.accepted_request_status_being_prepared);
